...
public class NNCheck : Agent
{
    ...
    private void OnCollisionEnter(Collision collision)
    {
        ...
        else if (collision.gameObject.CompareTag("Extremo"))
        {
            // Penalty for hitting other cars
            AddReward(wallCollisionPenalty/2);
        }        
    }
    ...
    // Método público para actualizar la referencia de TrackCheckpoints en caliente
    public void SetTrackCheckpoints(TrackCheckpoints newTrackCheckpoints)
    {
        // Desuscribir eventos del anterior (si existía)
        if (trackCheckpoints != null)
        {
            trackCheckpoints.OnCarCorrectCheckpoint -= OnCarCorrectCheckpoint;
            trackCheckpoints.OnCarWrongCheckpoint -= OnCarWrongCheckpoint;
        }
        trackCheckpoints = newTrackCheckpoints;
        // Suscribir eventos del nuevo (si no es null)
        if (trackCheckpoints != null)
        {
            trackCheckpoints.OnCarCorrectCheckpoint += OnCarCorrectCheckpoint;
            trackCheckpoints.OnCarWrongCheckpoint += OnCarWrongCheckpoint;
        }
        // Actualizar info de checkpoint
        UpdateNextCheckpointInfo();
    }
}
